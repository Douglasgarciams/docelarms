...
# Função upload_arquivo_para_b2 REMOVIDA
...
def anunciar_imovel(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            imovel.save() # django-storages cuida do upload aqui
            
            fotos_galeria = request.FILES.getlist('fotos_galeria')
            for f in fotos_galeria:
                Foto.objects.create(imovel=imovel, imagem=f) # django-storages cuida do upload aqui
            
            messages.success(request, 'Seu imóvel foi enviado para análise!') # Mensagem original
...
def excluir_imovel(request, imovel_id):
    imovel = get_object_or_404(Imovel, id=imovel_id, proprietario=request.user)
    if request.method == 'POST':
        # (Futura melhoria: excluir fotos do B2. Por enquanto, foca no upload)
        imovel.delete()
